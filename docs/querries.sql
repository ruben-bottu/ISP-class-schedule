...
WHERE EXISTS(
              ...
              FROM lessons s2
              WHERE s1.id <> s2.id
                AND (s1.start_timestamp, s1.end_timestamp) OVERLAPS
...
SELECT count(*) / 2 AS collision_count
FROM lessons s1
         INNER JOIN lessons s2 ON s1.id <> s2.id
...
CREATE
TEMPORARY TABLE IF NOT EXISTS selected_lessons AS
...
WHERE (s1.start_timestamp, s1.end_timestamp) OVERLAPS (s2.start_timestamp,
                                                       s2.end_timestamp) ===============================================================================================

CREATE
TEMPORARY TABLE input_data (
...
WHERE EXISTS(
              ...
              WHERE i.course_id = s.course_id
                AND i.class_group_id = s.class_group_id
          );
...
CREATE
OR REPLACE FUNCTION calculate_number_of_collisions(input_data course_class_group_ids)
RETURNS INT AS $$
	CREATE
TEMPORARY TABLE selected_lessons AS
SELECT id, start_timestamp, end_timestamp
FROM lessons s
WHERE EXISTS(
              SELECT '1'
...
CREATE
OR REPLACE FUNCTION testRowConstr(course_class_group_ids TEXT) RETURNS INT
	AS $body$
	DECLARE
sqlquery TEXT;
		collision_count
...
CREATE
OR REPLACE FUNCTION create_query(course_class_group_ids TEXT) RETURNS TEXT
	AS $$
SELECT FORMAT('SELECT count(*) FROM (VALUES %s) AS result_table(course_id, class_group_id)', $1) $$ LANGUAGE SQL;
...
-- count_collisions
CREATE
OR REPLACE FUNCTION testRowConstr(course_ids_class_group_ids TEXT) RETURNS INT
	AS $$
	DECLARE
collision_count INT;
BEGIN
...
CREATE
OR REPLACE FUNCTION collision_query(course_ids_class_group_ids TEXT) RETURNS TEXT
	AS $body$
SELECT FORMAT($$
		...
		SELECT id, start_timestamp, end_timestamp FROM lessons s
		WHERE EXISTS (
			SELECT '1'
...
FROM selected_lessons s1
         INNER JOIN selected_lessons s2 ON s1.id <> s2.id
WHERE (s1.start_timestamp, s1.end_timestamp) OVERLAPS
      (s2.start_timestamp, s2.end_timestamp) $$, course_ids_class_group_ids)
	$body$
	LANGUAGE SQL;


CREATE
OR REPLACE FUNCTION count_collisions(course_ids_class_group_ids TEXT) RETURNS INT
	AS $$
	DECLARE
collision_count INT;
BEGIN
...
CREATE
TEMP TABLE IF NOT EXISTS temp_tbl ON COMMIT DROP
AS
SELECT DISTINCT course_id, class_group_id
FROM lessons;

SELECT ARRAY[(s1.course_id, s1.class_group_id),
       (s2.course_id, s2.class_group_id),
       (s3.course_id, s3.class_group_id) ]
FROM temp_tbl s1 INNER JOIN temp_tbl s2
ON s2.course_id NOT IN (s1.course_id)
...
CREATE
OR REPLACE FUNCTION test2(course_id_class_group_id[]) RETURNS TEXT
 	AS $$
SELECT $1
           $$ LANGUAGE SQL;
...
CREATE
OR REPLACE FUNCTION test5(selected_courses_count INT) RETURNS TEXT
	AS $$
	DECLARE
select_clause	TEXT := 'SELECT CAST (ARRAY [(s1.course_id, s1.class_group_id)';
		from_clause
TEXT := 'FROM selected_courses_permutations s1';
BEGIN
		-- Construct SELECT
...
END LOOP;
		...
		select_clause
:= select_clause || '] AS course_id_class_group_id[]) AS permutation';
...
CREATE
OR REPLACE FUNCTION test7(input_course_id INT) RETURNS SETOF course_id_class_group_id
	AS $$
SELECT DISTINCT course_id, class_group_id
FROM lessons
...
CREATE
OR REPLACE FUNCTION test5(selected_course_ids INT ARRAY) RETURNS TEXT
	AS $$
	DECLARE
...
CREATE
OR REPLACE FUNCTION test6(ints INT ARRAY) RETURNS SETOF course_id_class_group_id ARRAY
	AS $$
BEGIN
...
CREATE
OR REPLACE FUNCTION get_class_groups_of_course(course_id INT) RETURNS SETOF course_id_class_group_id
	AS $$
SELECT DISTINCT course_id, class_group_id
FROM lessons
WHERE course_id = $1;
...
CREATE
OR REPLACE FUNCTION construct_generate_course_class_group_combinations_query(selected_course_ids INT ARRAY) RETURNS TEXT
	AS $$
	DECLARE
number_of_courses	INT	 		DEFAULT array_length(selected_course_ids, 1);
		select_array
...
CREATE
OR REPLACE FUNCTION generate_course_class_group_combinations(course_ids INT ARRAY) RETURNS SETOF course_id_class_group_id ARRAY
	AS $$
BEGIN
...
BEGIN
		-- ON COMMIT DROP can be removed if we work with pooling connections
		CREATE
TEMPORARY TABLE IF NOT EXISTS selected_lessons ON COMMIT DROP
AS
SELECT id, start_timestamp, end_timestamp
...
CREATE
OR REPLACE FUNCTION generate_course_class_group_combinations(course_ids INT ARRAY) RETURNS TABLE(combination course_id_class_group_id ARRAY)
	AS $$
BEGIN
RETURN QUERY EXECUTE construct_generate_course_class_group_combinations_query(course_ids);
END;
...
CREATE
OR REPLACE FUNCTION get_combinations_with_collision_count(row_limit INT, course_ids INT ARRAY)
...
CREATE
OR REPLACE FUNCTION get_combinations_with_collision_count_json(row_limit INT, course_ids INT ARRAY) RETURNS TEXT
	AS $$
...
SELECT json_agg(json_build_object('collision_count', collision_count, 'combination',
                                  (SELECT jsonb_agg(jsonb_build_object('course_name', c.name, 'class_group_name',
                                                                       cg.name))
                                   FROM unnest(combination)
                                            INNER JOIN class_groups cg ON class_group_id = cg.id
                                            INNER JOIN courses c ON course_id = c.id
                                   ORDER BY 1)))
FROM get_combinations_with_collision_count(row_limit, course_ids) $$ LANGUAGE SQL;
...
CREATE
OR REPLACE FUNCTION get_combinations_with_collision_count_json(row_limit INT, VARIADIC course_ids INT ARRAY) RETURNS TEXT
	AS $$
