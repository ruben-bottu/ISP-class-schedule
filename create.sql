...
CREATE OR REPLACE FUNCTION get_class_groups_of_course(course_id BIGINT) RETURNS SETOF course_id_class_group_id
	AS $$
...
CREATE OR REPLACE FUNCTION construct_generate_course_class_group_combinations_query(selected_course_ids BIGINT ARRAY) RETURNS TEXT
	AS $$
	DECLARE
...
CREATE OR REPLACE FUNCTION generate_course_class_group_combinations(course_ids BIGINT ARRAY) RETURNS TABLE(combination course_id_class_group_id ARRAY)
	AS $$
	BEGIN
...
CREATE OR REPLACE FUNCTION get_combinations_with_collision_count(row_limit INT, course_ids BIGINT ARRAY)
...
CREATE OR REPLACE FUNCTION get_combinations_with_collision_count_json(row_limit INT, VARIADIC course_ids BIGINT ARRAY) RETURNS TEXT
	AS $$
